"""
v27 气象数据版(修复) - 在v25基础上整合NWP气象数据
修复: 气象数据覆盖训练集(2025)和测试集(2026),之前只加载了2025年
"""
import pandas as pd
import numpy as np
import os
...
output_price_path = os.path.join(output_dir, 'v27_weather_price.csv')
output_power_path = os.path.join(output_dir, 'v27_weather_output.csv')
...
if __name__ == '__main__':
    print("=" * 60)
    print("v27 气象数据版(修复) - 整合完整NWP气象数据")
    print("=" * 60)
    ...
    print(f"  气象加载范围: {all_start.date()} ~ {all_end.date()}")

    # ==================== 2. 加载全部气象数据 ====================
    print("\n[2/6] 加载全部气象数据...")
    try:
        df_weather = load_weather_data(nc_dir, all_start, all_end)
        print(f"  气象数据加载成功: {len(df_weather)} 条 (15分钟粒度)")
    ...
    print("\n" + "=" * 60)
    print("Feature Importance (v27 气象数据版)")
    print("=" * 60)

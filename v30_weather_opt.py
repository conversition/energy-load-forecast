"""
v30 气象特征组合优化版 - 基于v29对照组实验的深度分析
核心优化:
1. 解决v29对照组实验中"RMSE最优但实际收益非最优"的矛盾
2. 测试风速+光照组合(去除温度) vs 全量气象
3. 引入相关性加权的特征选择策略
"""
import pandas as pd
import numpy as np
...
output_price_path = os.path.join(output_dir, 'v30_exp_price.csv')
output_power_path = os.path.join(output_dir, 'v30_exp_output.csv')
...
def plot_correlation_analysis(df_train, target_col='A'):
    """气象特征与电价相关性分析"""
    ...
    corr_matrix.to_csv('output/weather_correlation_v30.csv')
    logger.info("相关性矩阵已保存至 output/weather_correlation_v30.csv")

    return corr_matrix


def run_ablation_with_profit(df_train, y, split_idx, experiment_configs, base_features,
                             time_features, cyclical_features, business_features,
                             lag_features, stat_features, df_val_for_strategy):
    """带策略收益评估的对照组实验"""
    logger.info("\n========== 开始带收益评估的对照组实验 ==========")
    results_log = []
    ...
    for exp_name, added_features in experiment_configs.items():
        ...
        try:
            ...
            val_results = pd.DataFrame({
                'times': df_train['times'].iloc[split_idx:].reset_index(drop=True),
                'A': y_val_exp,
                'pred_A': y_val_pred_exp
            })
            profit = simulate_strategy_profit(val_results)
            ...
            logger.info(f"[{exp_name}] -> RMSE: {rmse_exp:.5f}, 收益: {profit:.0f}")
        except Exception as e:
            logger.warning(f"[{exp_name}] 实验失败: {e}")

    exp_df = pd.DataFrame(results_log).sort_values(by='Val_Profit', ascending=False)
    logger.info("\n========== 实验结果排行榜(按收益) ==========")
    logger.info("\n" + exp_df.to_string(index=False))
    return exp_df


def simulate_strategy_profit(val_df, noise_margin=0.0, threshold=0):
    """模拟策略收益"""
    val_df = val_df.copy()
    val_df['date'] = val_df['times'].dt.date
    total_profit = 0
    days_operated = 0
    ...
    for date, group in val_df.groupby('date'):
        ...
        for tc in range(0, 81):
            ...
            for td in range(tc + 8, 89):
                base_discharge = np.sum(prices[td:td+8]) * 1000
                worst_discharge = base_discharge * (1 - noise_margin)
                worst_profit = worst_discharge - worst_charge

                if worst_profit > best_worst_case:
                    best_worst_case = worst_profit
                    best_tc, best_td = tc, td

        if best_tc >= 0 and best_worst_case >= threshold:
...
if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("v30 气象特征组合优化版 - 深度分析RMSE与收益的矛盾")
    logger.info("=" * 60)
    ...
    # ==================== 4. 相关性分析 ====================
    logger.info("[4/7] 生成气象特征相关性分析...")
    corr_matrix = plot_correlation_analysis(df_train)

    # ==================== 5. 带收益评估的对照组实验 ====================
    logger.info("[5/7] 执行带收益评估的对照组实验...")
    ...
    df_val_for_strategy = pd.DataFrame({
        'times': df_train['times'].iloc[split_idx:].reset_index(drop=True),
        'A': y_val
    })

    exp_df = run_ablation_with_profit(
    ...
    # 找出收益最优的实验配置
    best_profit_exp = exp_df.iloc[0]['Experiment']
    best_rmse_exp = exp_df.sort_values(by='RMSE').iloc[0]['Experiment']
    logger.info(f"\n收益最优实验: {best_profit_exp}")
    logger.info(f"RMSE最优实验: {best_rmse_exp}")

    # ==================== 6. 使用最优配置训练完整模型 ====================
    logger.info("[6/7] 使用最优配置训练完整模型...")

    # 选择策略：收益最优 > RMSE最优
    # 因为我们的目标是最大化收益，不是最小化RMSE
    if best_profit_exp == "All Weather (全量)":
        final_weather_features = weather_features
    elif best_profit_exp == "+ 光照+温度":
        final_weather_features = ['ghi', 'temperature']
    elif best_profit_exp == "+ 风速+温度":
        final_weather_features = ['wind_speed', 'temperature']
    elif best_profit_exp == "+ 光照+风速":
        final_weather_features = ['ghi', 'wind_speed']
    elif best_profit_exp == "+ 仅温度 (temp)":
        final_weather_features = ['temperature']
    elif best_profit_exp == "+ 仅风速 (wind)":
        final_weather_features = ['wind_speed']
    elif best_profit_exp == "+ 仅光照 (ghi)":
        final_weather_features = ['ghi']
    else:
        final_weather_features = weather_features  # 默认全量

    logger.info(f"选择气象特征配置: {final_weather_features}")
    logger.info(f"理由: 基于收益评估而非RMSE选择")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("v30 实验结果分析:")
    logger.info("=" * 60)
    logger.info("关键发现: RMSE最优 ≠ 收益最优")
    logger.info("原因: 气象特征对电价预测有贡献,但对套利策略的贡献不直接")
    logger.info("因此选择基于验证集收益评估,而非RMSE")

    logger.info("\n" + "=" * 60)
    logger.info("Feature Importance (v30):")
    for feat, imp in sorted(zip(all_features, model.feature_importances_), key=lambda x: -x[1]):
        bar = "=" * int(imp * 100)
        logger.info(f"  {feat:<20s}: {imp:.4f} {bar}")

    logger.info("v30 气象特征组合优化版完成!")
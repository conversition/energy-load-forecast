"""
v36 稳健最终版 - 基于v31成功经验 + 改进特征选择
核心改进:
1. 使用真实验证收益进行特征选择
2. 保留v31验证通过的风速+温度核心配置
3. 增加更多气象特征候选组合
4. 去掉策略参数寻优(避免过拟合),使用固定稳健参数
"""
import pandas as pd
import numpy as np
import os
...
output_price_path = os.path.join(output_dir, 'v36_exp_price.csv')
output_power_path = os.path.join(output_dir, 'v36_exp_output.csv')
...
def evaluate_weather_configs(df_train, y, split_idx, base_features, time_features,
    ...
    configs = {
        ...
        "+Temp": ['temperature'],
        "+Ghi": ['ghi'],
        "+Wind+Temp": ['wind_speed', 'temperature'],
        "+Wind+Ghi": ['wind_speed', 'ghi'],
        "+Temp+Ghi": ['temperature', 'ghi'],
        "+All": ['temperature', 'ghi', 'wind_speed'],
    }

    results = []
    for name, weather_feats in configs.items():
        current_features = (base_features + time_features + cyclical_features +
                           business_features + lag_features + stat_features + weather_feats)
        ...
        try:
            ...
            results.append({
                ...
                'Weather': str(weather_feats)
            })
            logger.info(f"[{name}] -> RMSE: {rmse:.5f}, 真实验证收益: {true_profit:.0f}")
        except Exception as e:
            logger.warning(f"[{name}] 失败: {e}")

    if results:
        df_results = pd.DataFrame(results).sort_values(by='True_Profit', ascending=False)
...
if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("v36 稳健最终版 - 真实验证收益特征选择")
    logger.info("=" * 60)
    ...
    # ==================== 4. 气象配置评估 ====================
    logger.info("[4/5] 气象配置评估...")
    df_results = evaluate_weather_configs(
    ...
    if df_results is not None and len(df_results) > 0:
        ...
        logger.info(f"\n最优配置: {best_config_name}, 真实验证收益: {best_row['True_Profit']:.0f}")

        # 解析气象特征
        weather_str = best_row['Weather']
        if 'wind_speed' in weather_str and 'temperature' in weather_str:
            final_weather_features = ['wind_speed', 'temperature']
        elif 'wind_speed' in weather_str and 'ghi' in weather_str:
            final_weather_features = ['wind_speed', 'ghi']
        elif 'temperature' in weather_str and 'ghi' in weather_str:
            final_weather_features = ['temperature', 'ghi']
        elif 'wind_speed' in weather_str:
            final_weather_features = ['wind_speed']
        elif 'temperature' in weather_str:
            final_weather_features = ['temperature']
        elif 'ghi' in weather_str:
            final_weather_features = ['ghi']
        else:
            final_weather_features = []
    else:
        final_weather_features = ['wind_speed', 'temperature']
        logger.info("使用默认配置: wind_speed + temperature")

    logger.info(f"选择的气象特征: {final_weather_features}")

    # ==================== 5. 训练和预测 ====================
    logger.info("[5/5] 训练模型并预测...")

"""
v35 策略优化版 - 基于v31配置 + 验证集策略参数寻优
核心优化:
1. 使用v31验证通过的风速+温度配置
2. 在验证集上优化策略参数(noise_margin, threshold)
3. 使用真实验证收益选择最优策略参数
"""
import pandas as pd
import numpy as np
...
output_price_path = os.path.join(output_dir, 'v35_exp_price.csv')
output_power_path = os.path.join(output_dir, 'v35_exp_output.csv')
...
def optimize_strategy_params(y_val_pred, y_val):
    """在验证集上优化策略参数"""
    logger.info("\n========== 策略参数寻优 ==========")

    best_profit = -float('inf')
    best_noise, best_threshold = 0.0, 0

    for noise in [0.0, 0.05, 0.10, 0.15, 0.20]:
        for threshold in [0, 1000, 2000, 3000, 5000]:
            profit = simulate_true_validation_profit(y_val_pred, y_val, noise, threshold)
            if profit > best_profit:
                best_profit = profit
                best_noise, best_threshold = noise, threshold
                logger.info(f"  noise={noise:.2f}, threshold={threshold} -> 收益: {profit:.0f}")

    logger.info(f"\n最优策略参数: noise={best_noise}, threshold={best_threshold}")
    return best_noise, best_threshold, best_profit
...
if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("v35 策略优化版 - 风速+温度 + 策略参数寻优")
    logger.info("=" * 60)
    ...
    # ==================== 4. 训练模型 ====================
    logger.info("[4/6] 训练模型...")

    all_features = (base_features + time_features + cyclical_features +
                   business_features + lag_features + stat_features + weather_features)
    logger.info(f"特征配置 ({len(all_features)}个): {weather_features}")
    ...
    # ==================== 5. 策略参数寻优 ====================
    logger.info("[5/6] 策略参数寻优...")
    best_noise, best_threshold, val_profit = optimize_strategy_params(y_val_pred, y_val)
    ...
    X_test_base = df_test[base_features + time_features + cyclical_features +
                         business_features + weather_features].copy()

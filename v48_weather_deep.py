...
v48: 气象特征深度挖掘（第二优先级）
核心改进:
1. 新增GHI(水平面总辐照度) - 光伏出力关键特征
2. 新增TCC(总云量) - 影响光伏和用电需求
3. 新增MSL(海平面气压) - 天气系统指标
4. 新增辐照度/云量的导数特征
"""
import pandas as pd
import numpy as np
...
output_price_path = os.path.join(output_dir, 'v48_weather_deep_price.csv')
output_power_path = os.path.join(output_dir, 'v48_weather_deep_output.csv')
...
if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("v48: 气象特征深度挖掘（第二优先级）")
    logger.info("新增: GHI/TCC/MSL及交互/导数特征")
    logger.info("=" * 60)

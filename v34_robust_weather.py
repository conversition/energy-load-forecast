"""
v34 稳健气象特征版 - 基于v33交叉验证结果
核心优化:
1. 交叉验证结果: wind_speed平均收益589833,最优
2. 固定80/20分割验证,确保与v28可比较
3. 使用真实验证收益评估(预测决策+真实结算)
"""
import pandas as pd
import numpy as np
import os
...
output_price_path = os.path.join(output_dir, 'v34_exp_price.csv')
output_power_path = os.path.join(output_dir, 'v34_exp_output.csv')
...
def compare_weather_configs(df_train, y, split_idx, base_features, time_features,
    ...
    configs = {
        ...
        "+ 风速+光照": ['wind_speed', 'ghi'],
        "+ 全量气象": ['temperature', 'ghi', 'wind_speed'],
    }
    ...
    for name, weather_feats in configs.items():
        ...
        try:
            ...
            results.append({
                ...
                'Weather_Feats': str(weather_feats)
            })
...
if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("v34 稳健气象特征版 - 真实验证收益评估")
    logger.info("=" * 60)
    ...
    # ==================== 4. 气象配置比较 ====================
    logger.info("[4/6] 气象配置比较(真实验证收益)...")
    best_config = compare_weather_configs(
        df_train, y, split_idx, base_features, time_features,
        cyclical_features, business_features, lag_features, stat_features
    )

    if best_config is not None:
        weather_feats_str = best_config['Weather_Feats']
        # 解析气象特征
        if 'wind_speed' in weather_feats_str and 'temperature' in weather_feats_str:
            final_weather_features = ['wind_speed', 'temperature']
        elif 'wind_speed' in weather_feats_str and 'ghi' in weather_feats_str:
            final_weather_features = ['wind_speed', 'ghi']
        elif 'wind_speed' in weather_feats_str:
            final_weather_features = ['wind_speed']
        elif 'temperature' in weather_feats_str and 'ghi' in weather_feats_str:
            final_weather_features = ['temperature', 'ghi']
        elif 'temperature' in weather_feats_str:
            final_weather_features = ['temperature']
        elif 'ghi' in weather_feats_str:
            final_weather_features = ['ghi']
        else:
            final_weather_features = []

        logger.info(f"\n最优配置: {best_config['Config']}")
        logger.info(f"气象特征: {final_weather_features}")
    else:
        final_weather_features = ['wind_speed', 'temperature']

...
核心优化:
1. 基于 v42 的精细特征工程
2. 优化风险因子: 允许更细致的 risk_factor 搜索
3. 动态阈值优化: 根据价格波动率自适应调整
4. 充电时段优化: 考虑分时电价选择最佳充电窗口
"""
import pandas as pd
import numpy as np
import os
...
output_price_path = os.path.join(output_dir, 'v43_strategy_price.csv')
output_power_path = os.path.join(output_dir, 'v43_strategy_output.csv')

target_col = 'A'
LAG_48, LAG_96, LAG_192, LAG_288, LAG_576 = 48, 96, 192, 288, 576
...
def compute_deviation_features(df_feat, deviation_cols):
    ...
    for col in deviation_cols:
        if col in deviation_stats:
            ...
            df_feat_out[f'{col}_dev_std'] = df_feat_out.apply(
                lambda row: deviation_stats[col]['std'].get((row['hour'], row['month']), 0), axis=1
            )
...
def prepare_train_data(df_feat, df_label, df_weather, weather_means):
    ...
    if not df_weather.empty:
        ...
        for col in ['temperature', 'wind_speed', 'ghi']:
            df[col] = df[col].fillna(weather_means.get(col, 0))
    df = add_time_features(df)
    df = add_cyclical_features(df)
    df = add_interaction_features(df)

    for lag in [48, 96, 192, 288]:
    ...
    return df.iloc[LAG_288:].reset_index(drop=True)
...
def train_quantile_ensemble_models(X_train, y_train):
    ...
    models['lgb_p50'] = lgb.LGBMRegressor(
        n_estimators=400, learning_rate=0.025, max_depth=8, num_leaves=63,
...
def find_best_params_fine(p10_val, p50_val, p90_val, y_val):
    """精细搜索最优 risk_factor 和噪声因子"""
    df_val = pd.DataFrame({'p10': p10_val, 'p90': p90_val, 'median': p50_val})
    df_val['date'] = [i // 96 for i in range(len(df_val))]

    best_profit, best_rf = -float('inf'), 0.3
    for rf in np.arange(0.1, 1.0, 0.1):
        profit = 0
        ...
        logger.info(f"    rf={rf:.1f} -> 收益: {profit:.0f}")
...
def generate_optimized_strategy(price_p10, price_p90, price_median, save_path, risk_factor=0.3):
    """优化的分位数策略"""
    ...
    logger.info(f"  执行策略: rf={risk_factor}, 操作 {days_operated} 天")
    return total_profit


if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("v43 策略优化版 - 精细特征 + 优化风险因子")
    logger.info("=" * 60)
    ...
    interaction_features = ['供需比', '风光差', '净负荷', '风光渗透率', 'is_workday', '负荷率', '风光比']
    lag_features = ['price_lag_48', 'price_lag_96', 'price_lag_192', 'price_lag_288']
    stat_features = ['rolling_mean', 'rolling_std']
    weather_features = ['wind_speed', 'temperature', 'ghi']

    deviation_cols = ['系统负荷', '风光总加', '风电', '光伏']
    deviation_feature_names = [f'{col}_dev_mean' for col in deviation_cols] + [f'{col}_dev_std' for col in deviation_cols]
    ...
    weather_means = {col: df_weather[col].mean() for col in ['temperature', 'wind_speed', 'ghi']} if not df_weather.empty else {}

    # 2. 计算偏差特征
    logger.info("[2/7] 计算偏差特征...")
    ...
    # 6. 精细搜索最优参数
    logger.info("[6/7] 精细搜索最优参数...")
    best_rf, best_profit = find_best_params_fine(p10_val, p50_val, p90_val, y_val)
    logger.info(f"  最优 rf={best_rf:.1f}, 验证收益={best_profit:.0f}")
    ...
    for col in deviation_cols:
        if col in deviation_stats:
            df_test_raw[f'{col}_dev_mean'] = df_test_raw.apply(
                lambda row: deviation_stats[col]['mean'].get((row['hour'], row['month']), 0), axis=1
            )
            df_test_raw[f'{col}_dev_std'] = df_test_raw.apply(
                lambda row: deviation_stats[col]['std'].get((row['hour'], row['month']), 0), axis=1
            )
    ...
    all_predicted = np.concatenate([df_train[target_col].values[-LAG_288:].copy()])
    ...
    for day_idx in range(n_days):
        ...
        history = pd.Series(all_predicted[-LAG_576:])
        lag_48 = history.shift(48).values[-LAG_96:]
        lag_96 = history.shift(LAG_96).values[-LAG_96:]
        lag_192 = history.shift(LAG_192).values[-LAG_96:]
        lag_288 = history.shift(LAG_288).values[-LAG_96:]
    ...
    total_profit = generate_optimized_strategy(
        predicted_p10, predicted_p90, predicted_p50,
        output_power_path, risk_factor=best_rf
    )

    logger.info("\n" + "=" * 60)
    logger.info(f"v43 策略优化版完成!")
    logger.info(f"最优 rf: {best_rf:.1f}")

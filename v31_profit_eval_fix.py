"""
v31 修正验证收益评估版 - 解决v30发现的问题
核心修复:
1. 验证集收益评估必须使用预测价格,而非真实价格
2. 修正对照实验的收益计算逻辑
3. 基于修正后的评估选择最优特征配置
"""
import pandas as pd
...
output_price_path = os.path.join(output_dir, 'v31_exp_price.csv')
output_power_path = os.path.join(output_dir, 'v31_exp_output.csv')
...
def simulate_strategy_profit_on_predicted(prices_pred, noise_margin=0.0, threshold=0):
    """在预测价格上模拟策略收益"""
    ...
    for day_idx in range(n_days):
        ...
        for tc in range(0, 81):
            base_charge = np.sum(day_prices[tc:tc+8]) * 1000
            worst_charge = base_charge * (1 + noise_margin)

            for td in range(tc + 8, 89):
                base_dis = np.sum(day_prices[td:td+8]) * 1000
        ...
        if best_tc >= 0 and best_worst >= threshold:
            nominal = (np.sum(day_prices[best_td:best_td+8]) - np.sum(day_prices[best_tc:best_tc+8])) * 1000
...
def run_ablation_with_predicted_profit(df_train, y, split_idx, experiment_configs, base_features,
    ...
    for exp_name, added_features in experiment_configs.items():
        ...
        try:
            ...
            # 使用预测价格计算验证集策略收益
            val_profit = simulate_strategy_profit_on_predicted(y_val_pred_exp)
            ...
            logger.info(f"[{exp_name}] -> RMSE: {rmse_exp:.5f}, 预测收益: {val_profit:.0f}")
...
if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("v31 修正验证收益评估版 - 基于预测价格评估")
    logger.info("=" * 60)
    ...
    # ==================== 4. 对照组实验(使用预测价格评估) ====================
    logger.info("[4/6] 执行使用预测价格评估的对照组实验...")
    ...
    exp_df = run_ablation_with_predicted_profit(
    ...
    logger.info(f"\n预测收益最优实验: {best_profit_exp}")
    logger.info(f"RMSE最优实验: {best_rmse_exp}")

    # ==================== 5. 使用最优配置训练完整模型 ====================
    logger.info("[5/6] 使用最优配置训练完整模型...")
    ...
    logger.info("\n" + "=" * 60)
    logger.info("v31 实验结果:")
    logger.info("=" * 60)
    logger.info(f"预测收益最优配置: {best_profit_exp}")
    logger.info(f"最终气象特征: {final_weather_features}")

    logger.info("\n" + "=" * 60)
    logger.info("Feature Importance (v31):")
    for feat, imp in sorted(zip(all_features, model.feature_importances_), key=lambda x: -x[1]):
        bar = "=" * int(imp * 100)
        logger.info(f"  {feat:<20s}: {imp:.4f} {bar}")

    logger.info("v31 修正验证收益评估版完成!")
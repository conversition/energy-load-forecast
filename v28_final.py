...
output_price_path = os.path.join(output_dir, 'v28_final_price.csv')
output_power_path = os.path.join(output_dir, 'v28_final_output.csv')
...
def load_weather_data_fixed(nc_dir, start_date, end_date):
    ...
    for nc_file in nc_files:
        ...
        try:
            ...
            logger.warning(f"无法解析日期: {filename}, 跳过")
            continue
        ...
        try:
            ...
            # channel: 0=ghi, 2=t2m(开尔文), 5=u100, 6=v100
            ghi = data_mean[:, 0]
            t2m = data_mean[:, 2] - 273.15  # 开尔文转摄氏度
            u100 = data_mean[:, 5]
            v100 = data_mean[:, 6]
            wind_speed = np.sqrt(u100**2 + v100**2)

            # 【修复时区】直接使用UTC时间，避免与训练数据时间对齐问题
            # UTC时间00:00对应北京时区00:00（内蒙古地区）
            ...
            logger.warning(f"解析文件 {filename} 失败: {e}, 跳过")
            continue

    if not weather_list:
        logger.warning("未找到任何气象数据")
        return pd.DataFrame()
    ...
    logger.info(f"气象数据加载完成: {len(df_weather_15min)} 条")
...
def simulate_strategy_profit(val_df, noise_margin, threshold):
    """在验证集上模拟策略收益"""
    val_df = val_df.copy()
...
def generate_strategy(price_csv, save_path, noise_margin=0.0, threshold=0):
    ...
    print(f"  策略: noise={noise_margin}, threshold={threshold}, 操作{days_operated}天")
    return total_profit


if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("v28 终极版 - 启动")
    logger.info("=" * 60)
    ...
    best_noise, best_threshold = 0, 0

    for noise in [0.0, 0.05, 0.10, 0.15]:
        for threshold in [0, 1000, 3000, 5000]:
            profit = simulate_strategy_profit(val_results, noise, threshold)
            if profit > best_val_profit:
                best_val_profit = profit
                best_noise, best_threshold = noise, threshold

    logger.info(f"验证集最优参数锁定: noise={best_noise}, threshold={best_threshold}, profit={best_val_profit:.2f}")
    ...
    for day_idx in range(n_days):
        ...
        # 取值并填充NaN（第一天的lag_192_series可能有NaN）
    ...
    total_profit = generate_strategy(output_price_path, output_power_path,
                                    noise_margin=best_noise, threshold=best_threshold)
    logger.info(f"总收益: {total_profit:.2f}, 日均: {total_profit/n_days:.2f}")

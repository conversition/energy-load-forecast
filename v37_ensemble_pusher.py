"""
v37 终极冲分版 - 双模型融合 (GBDT + LightGBM) + 黄金气象特征
核心杀器:
1. 锁定 wind_speed + temperature
2. 异构模型加权融合，中和过拟合风险，捕捉极端极值
3. 基于融合曲线的真实验证收益评估与策略参数锁定
"""
import pandas as pd
import numpy as np
import os
...
output_price_path = os.path.join(output_dir, 'v37_ensemble_price.csv')
output_power_path = os.path.join(output_dir, 'v37_ensemble_output.csv')
...
if __name__ == '__main__':
    logger.info("=" * 60)
    logger.info("v37 冲分版 - 锁定黄金特征 + 双模型异构融合")
    logger.info("=" * 60)
    ...
    # 3. 验证集评估与融合
    logger.info("[3/6] 验证集评估与融合...")
    pred_gbdt = model_gbdt.predict(X_val)
    pred_lgb = model_lgb.predict(X_val)

    weight_gbdt, weight_lgb = 0.5, 0.5
    y_val_pred_ensemble = (pred_gbdt * weight_gbdt) + (pred_lgb * weight_lgb)

    rmse_gbdt = np.sqrt(mean_squared_error(y_val, pred_gbdt))
    rmse_lgb = np.sqrt(mean_squared_error(y_val, pred_lgb))
    rmse_ens = np.sqrt(mean_squared_error(y_val, y_val_pred_ensemble))

    true_profit = simulate_true_validation_profit(y_val_pred_ensemble, y_val)

    logger.info(f"  GBDT RMSE: {rmse_gbdt:.6f}")
    logger.info(f"  LightGBM RMSE: {rmse_lgb:.6f}")
    logger.info(f"  融合 RMSE: {rmse_ens:.6f}")
    logger.info(f"  真实验证收益: {true_profit:.0f}")

    # 4. 验证集策略参数寻优
    logger.info("[4/6] 动态寻找最稳策略参数...")
    best_profit, best_noise, best_threshold = -float('inf'), 0, 0
    for noise in [0.0, 0.05, 0.10]:
        for threshold in [0, 1000, 3000]:
            profit = simulate_true_validation_profit(y_val_pred_ensemble, y_val, noise, threshold)
            if profit > best_profit:
                best_profit, best_noise, best_threshold = profit, noise, threshold

    logger.info(f"  -> 锁定参数: Noise={best_noise}, Threshold={best_threshold} (真实验证收益: {best_profit:.0f})")
    ...
    for day_idx in range(n_days):
        ...
        day_pred_ensemble = (day_pred_gbdt * weight_gbdt) + (day_pred_lgb * weight_lgb)
    ...
    logger.info("\n" + "=" * 60)
    logger.info(f"v37 冲分版完成!")
    logger.info(f"测试集总收益: {total_profit:.2f}, 日均: {total_profit/n_days:.2f}")
    logger.info("=" * 60)

    logger.info("\nFeature Importance (GBDT):")
    for feat, imp in sorted(zip(all_features, model_gbdt.feature_importances_), key=lambda x: -x[1])[:10]:
        bar = "=" * int(imp * 100)
        logger.info(f"  {feat:<20s}: {imp:.4f} {bar}")

    logger.info("\nFeature Importance (LightGBM):")
    for feat, imp in sorted(zip(all_features, model_lgb.feature_importances_), key=lambda x: -x[1])[:10]:
        bar = "=" * int(imp * 100)
        logger.info(f"  {feat:<20s}: {imp:.4f} {bar}")
"""
v21 修复版 - 彻底修复三大致命Bug
方案一: 修复target leakage - 基于price_lag_96计算滚动统计量
方案二: 修复train-test skew - 动态滚动均值替代直线填充
方案三: 释放模型能力 - 删除max_features='sqrt'
"""
import pandas as pd
import numpy as np
import os
...
output_price_path = os.path.join(output_dir, 'v21_fixed_price.csv')
output_power_path = os.path.join(output_dir, 'v21_fixed_output.csv')
...
def add_rolling_stats(df, price_col):
    """添加滚动统计量 - 核心修复: 基于price_lag_96计算，避免target leakage

    修复前(错误): 基于当前时刻price_col计算rolling，包含当天的数据
    修复后(正确): 基于price_lag_96计算，只用到过去的数据
    """
    df = df.copy()
...
if __name__ == '__main__':
    ...
    for day_idx in range(n_days):
        ...
        if day_idx < 5 or day_idx == n_days - 1:
            print(f"  Day {day_idx+1}: mean={day_pred.mean():.4f}, "
                  f"rolling_mean[0]={rolling_mean_arr[0]:.4f}, rolling_std[0]={rolling_std_arr[0]:.4f}")
    ...
    # 特征重要性
    print("\n" + "=" * 60)
    print("Feature Importance (v21 修复版)")
    print("=" * 60)
